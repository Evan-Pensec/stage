...
if (is.null(colonne_condition)) {
  cat("⚠️ Aucune colonne de condition trouvée. Création d'une division aléatoire WT/KO pour démonstration.\n")
  set.seed(123)
...
```{r creation_graphiques_compares, echo=FALSE}
...
creer_graphique_umap <- function(data, titre, condition) {
  plot_ly(
    ...
    showlegend = if(condition == "WT") TRUE else FALSE  # Afficher la légende seulement pour WT
  ) %>%
  layout(
    title = list(
...
# Créer les graphiques WT et KO
graphique_wt <- creer_graphique_umap(donnees_wt, "WT (Wild Type)", "WT")
graphique_ko <- creer_graphique_umap(donnees_ko, "KO (Knockout)", "KO")

# Combiner les graphiques en utilisant subplot
graphique_combine <- subplot(
  graphique_wt, graphique_ko,
  nrows = 1, 
  shareY = TRUE,
  titleX = TRUE,
  margin = 0.05
...
layout(
  title = list(
    text = paste("<b>Comparaison UMAP des Types Cellulaires : WT vs KO</b><br>",
                 "<i>Total:", format(nrow(mes_donnees), big.mark = " "), 
                 "cellules •", length(types_uniques), "types cellulaires</i>"),
    font = list(size = 18)
  ),
  legend = list(
    orientation = "v",
  ...
  margin = list(r = 200, l = 50, t = 100, b = 50)
)
...
# Section à ajouter dans votre fichier R Markdown après la section de chargement des données
